...
public class Damage : MonoBehaviour
{
    ...
    public bool immune_fire = false;
    public bool immune_ice = false;
    public bool immune_wind = false;
    ...
    private bool checkDamage(Collision collision)
    {
        Attributes attribute = collision.gameObject.GetComponentInParent<Attributes>();

        if (immune_fire && attribute.fire)
        {
            Destroy(this.gameObject);
            return false;
        }

        if (immune_ice && attribute.ice)
        {
            Destroy(this.gameObject);
            return false;
        }

        if (immune_wind && attribute.wind)
        {
            Destroy(this.gameObject);
            return false;
        }
    ...
    private bool checkDamage(Collider collision)
    {
        Attributes attribute = collision.gameObject.GetComponentInParent<Attributes>();
