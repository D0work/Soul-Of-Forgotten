...
{
    // the character rigidbody used for player motion
    ...
    public enum PlayerState { Idle, Moving, Jumping, Flying, Praying, RCasting, BCasting, LCasting, Dead };
    ...
    [Tooltip("The input action(s) that map to flying")]
    public InputAction flyInput;

    public bool isFlying = false;
    [Tooltip("The input action(s) that map to player attack activating")]
    public InputAction CastInput;
    [Tooltip("The input action(s) that map to player effect activating")]
    public InputAction BuffInput;
    [Tooltip("The input action(s) that map to envrionment effect activating")]
    public InputAction InvokeInput;
    ...
    void InitialSetup()
    {
        ...
        {
            Debug.LogError("There is no rigidbody attached to the same gameobject as the ThirdPersonCharacterController. + " +
                "\n It needs one to run correctly");
        }
    ...
    void LateUpdate()
    {
        ...
        if (CastInput.IsPressed())
        {
            playerState = PlayerState.RCasting;
        }
        else if (BuffInput.IsPressed())
        {
            playerState = PlayerState.BCasting;
        }
        else if (InvokeInput.IsInProgress())
        {
            playerState = PlayerState.LCasting;
        }
        else
        {
            isRunning = moveInput.IsPressed();
    ...
    void NormalControl(float leftRightMovementAxis, float forwardBackwardMovementAxis, bool jumpPressed)
    {
        ...
        if (characterController.isGrounded && !bounced)
        {
            ...
            if (isFlying)
            {
                ...
                moveDirection = new Vector3(leftRightInput, verticalInput, forwardBackwardInput);
                moveDirection = transform.TransformDirection(moveDirection);
